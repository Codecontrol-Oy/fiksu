...
const Select = props => {
    ...
    return <select required={props.required} style={props.style} value={props.value} id={props.id} name={props.name}  {...(props.onChange && { onChange: (e) => onChange(e) })} className={"input " + (classes.join(' '))}>{props.children}</select>
}
