...
import { RemoveFriendForm } from '../components';
...
export default class Friends extends React.PureComponent {
  ...
  mutation AddFriend($nickname: String!) {
                    ...
                    if (data.getFriends.length === 0) return (
                      <Mutation mutation={ADD_FRIEND} refetchQueries={['GetFriends']} >
                        {(addFriend, { data, error, loading }) => {
                          ...
                          console.log(data)
                          return (
                            <AddFriendForm error noFriends onSubmit={(nickname) => addFriend({ variables: { nickname: nickname } })} noValidate autoComplete="false" />
                          )
                        }}
                      </Mutation>
                    );
                      ...
                      return (<Grid key={friend.nickname} item md={3}>
                        <Card style={{ maxWidth: '270px' }}>
                          ...
                          <CardActions>
                            ...
                            <Mutation mutation={REMOVE_FRIEND} refetchQueries={['GetFriends']} >
                              {(unFriend, { data, error, loading }) => {
                                if (loading) return <p>Loading...</p>;
                                console.log(data)
                                return (
                                  <RemoveFriendForm onSubmit={(nickname) => { 
                                    console.log(nickname)
                                    unFriend({ variables: { friendId: friend._id } })
            ...
            <Card style={{ backgroundColor: colors.backgroundLight, padding: '8px' }}>
              <Mutation mutation={ADD_FRIEND} refetchQueries={['GetFriends']} >
                {(addFriend, { data, error, loading }) => {
                  ...
                  console.log(data)
                  return (
                    <AddFriendForm onSubmit={(nickname) => addFriend({ variables: { nickname: nickname } })} noValidate autoComplete="false" />
                  )
                }}
              </Mutation>
            </Card>
